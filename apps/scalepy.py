...
with inputCol:
    responseTab, selectionTab, scalingTab = st.tabs(
        ["Response Spectrum", "Record Filtering", "Scaling"])
    with responseTab:
        responseDefinitionType = st.selectbox(
            "Spectrum Definition", ['TBEC-2018', 'ASCE7-22', 'User-Defined'])
        if responseDefinitionType == 'TBEC-2018':
            with st.form("tbecForm"):
                Ss = st.number_input(
                    'Spectral Acceleration at Short Periods (Ss)', value=1.2)
                S1 = st.number_input(
                    'Spectral Acceleration at 1 sec (S1)', value=0.25)
                soil = st.selectbox(
                    'Soil Type', ('ZA', 'ZB', 'ZC', 'ZD', 'ZE'), 2)
                responseButton = st.form_submit_button(
                    'Create Response Spectrum')
        if responseDefinitionType == 'ASCE7-22':
            with st.form("asceForm"):
                responseType = st.selectbox("Spectrum Type", ('Multi Period Design Spectrum',
                                            'Multi Period MCEr Design Spectrum', 'Two Period Design Spectrum', 'Two Period MCEr Design Spectrum'))
                latitude = st.number_input(
                    'Latitude of Location', 26.0, 50.0, 32.0, 1.0)
                longitude = st.number_input(
                    'Longitude of Location', -125.0, -65.0, -94.0, 1.0)
                asceSoil = st.selectbox(
                    "Soil Type", ('A', 'B', 'C', 'D', 'E'), 2)
                asceResponseButton = st.form_submit_button(
                    'Create Response Spectrum')
        if responseDefinitionType == 'User-Defined':
            ...
            st.download_button("Example CSV file", data=exampleCSV,
                               file_name='exampleSpectrum.csv', mime='text/csv')
            with st.form("userDefinedForm"):
                userDefinedSpectrum = st.file_uploader(
                    'Upload Response Spectrum Data', type='csv', accept_multiple_files=False)
                st.set_option('deprecation.showfileUploaderEncoding', False)
                userDefinedSpectrumButton = st.form_submit_button(
                    'Create Response Spectrum')

    with selectionTab:
        with st.form("selectionForm"):
            period = st.number_input("Structure Period", 0.0, 10.0, 1.0, 0.1)
            magnitudeRange = st.slider(
                'Magnitude Range', 0.0, 12.0, (3.0, 10.0), step=0.2)
            vs30Range = st.slider('Vs30 Range', 0, 1500, (360, 760), step=10)
            rjbRange = st.slider('RJB Range', 0, 3000, (0, 3000), step=10)
            faultMechanism = st.selectbox('Fault Mechanism', [
                                          "Strike - Slip", "Normal", "Reverse", "Oblique", "Reverse - Oblique", "Normal - Oblique"])
            duration575Range = st.slider(
                '%5-%75 Duration Range', 0, 500, (0, 500), step=5)
            duration595Range = st.slider(
                '%5-%95 Duration Range', 0, 500, (0, 500), step=5)
            ariasIntensity = st.slider(
                'Arias Intensity Range', 0, 10, (0, 10), step=1)
            filterButton = st.form_submit_button("Filter Ground Motions")
            numberRecords = st.number_input(
                "Number of Ground Motions to be Scaled", 1, value=11, step=1)
            selectButton = st.form_submit_button(
                "Find Optimum Selected Ground Motions")

    with scalingTab:
        with st.form("scalingForm"):
            spectralOrdinate = st.selectbox(
                "Spectral Ordinate", ["SRSS", "RotD50", "RotD100"])
            targetShift = st.number_input(
                "Target Spectrum Shift", 0.1, value=1.3, step=0.1)
            periodRange = st.slider(
                "Period Range of Interest Coefficients", 0.1, 3.0, (0.2, 1.5), 0.1)
            scaleButton = st.form_submit_button("Perform Amplitude Scaling")

if responseDefinitionType == 'TBEC-2018':
    if responseButton:
        defaultTarget = scalepyBack.targetSpectrum(Ss, S1, soil)
        defaultTarget = defaultTarget.rename(
            columns={'T': 'Period (sec)', 'Sa': 'pSa (g)'})
        defaultFig = go.Figure()
...
if responseDefinitionType == 'ASCE7-22':
    if asceResponseButton:
        ...
        if responseType == 'Multi Period Design Spectrum':
            asceInit = accesAsce.getAsceDataMulti(
                latitude, longitude, 'III', asceSoil, 'Call')
            targetAsce = pd.DataFrame()
            targetAsce['multiPeriodDesignSpectrumPeriods'] = t
            targetAsce['multiPeriodDesignSpectrumOrdinates'] = [x*9.81 for x in np.interp(
                t, asceInit['multiPeriodDesignSpectrumPeriods'], asceInit['multiPeriodDesignSpectrumOrdinates'])]
            targetAsce = targetAsce.rename(columns={
                                           'multiPeriodDesignSpectrumPeriods': 'T', 'multiPeriodDesignSpectrumOrdinates': 'Sa'})

        elif responseType == 'Multi Period MCEr Design Spectrum':
            asceInit = accesAsce.getAsceDataMultiMCEr(
                latitude, longitude, 'III', asceSoil, 'Call')
            targetAsce = pd.DataFrame()
            targetAsce['multiPeriodMCErSpectrumPeriods'] = t
            targetAsce['multiPeriodMCErSpectrumOrdinates'] = [x*9.81 for x in np.interp(
                t, asceInit['multiPeriodMCErSpectrumPeriods'], asceInit['multiPeriodMCErSpectrumOrdinates'])]
            targetAsce = targetAsce.rename(columns={
                                           'multiPeriodMCErSpectrumPeriods': 'T', 'multiPeriodMCErSpectrumOrdinates': 'Sa'})

        elif responseType == 'Two Period Design Spectrum':
            asceInit = accesAsce.getAsceDataTwo(
                latitude, longitude, 'III', asceSoil, 'Call')
            targetAsce = pd.DataFrame()
            targetAsce['twoPeriodDesignSpectrumPeriods'] = t
            targetAsce['twoPeriodDesignSpectrumOrdinates'] = [x*9.81 for x in np.interp(
                t, asceInit['twoPeriodDesignSpectrumPeriods'], asceInit['twoPeriodDesignSpectrumOrdinates'])]
            targetAsce = targetAsce.rename(columns={
                                           'twoPeriodDesignSpectrumPeriods': 'T', 'twoPeriodDesignSpectrumOrdinates': 'Sa'})

        elif responseType == 'Two Period MCEr Design Spectrum':
            asceInit = accesAsce.getAsceDataTwoMCEr(
                latitude, longitude, 'III', asceSoil, 'Call')
            targetAsce = pd.DataFrame()
            targetAsce['twoPeriodMCErSpectrumPeriods'] = t
            targetAsce['twoPeriodMCErSpectrumOrdinates'] = [x*9.81 for x in np.interp(
                t, asceInit['twoPeriodMCErSpectrumPeriods'], asceInit['twoPeriodMCErSpectrumOrdinates'])]
            targetAsce = targetAsce.rename(
                columns={'twoPeriodMCErSpectrumPeriods': 'T', 'twoPeriodMCErSpectrumOrdinates': 'Sa'})
...
if responseDefinitionType == 'User-Defined':
    if userDefinedSpectrumButton:
        if userDefinedSpectrum is None:
            ...
            spectrumData = spectrumData.loc[:, ~spectrumData.columns.str.contains(
                '^Unnamed')]
...
if scaleButton:
    ...
    if spectralOrdinate == "SRSS":
        ...
        sf_dict, multiplied_selected_x, multiplied_selected_y, geo_mean_1st_scaled_df, srss_mean_df, srss_mean_scaled_df = scalepyBack.amplitudeScaling(
            selected_keys, selectedTarget, period, targetShift, periodRange[0], periodRange[1], 'srss')
    ...
    if spectralOrdinate == "RotD50":
        sf_dict, multiplied_selected_x, multiplied_selected_y, geo_mean_1st_scaled_df, rotd50_mean_df, rotd50_mean_scaled_df = scalepyBack.amplitudeScaling(
            selected_keys, selectedTarget, period, targetShift, periodRange[0], periodRange[1], 'rotd50')
    ...
    if spectralOrdinate == "RotD100":
        sf_dict, multiplied_selected_x, multiplied_selected_y, geo_mean_1st_scaled_df, rotd100_mean_df, rotd100_mean_scaled_df = scalepyBack.amplitudeScaling(
            selected_keys, selectedTarget, period, targetShift, periodRange[0], periodRange[1], 'rotd100')
...
if selectButton:
    defaultFrame = pd.DataFrame(columns=["Record Sequence Number", "Earthquake Name", "Station Name",
                                "Scale Factor"], index=pd.RangeIndex(start=1, stop=numberRecords + 1, name='index'))
